...
const unsigned long FAILSAFE_TIMEOUT_MS = 1000; // Stops motors if no command in 1000ms
bool isStopped = true;
...
void loop() {
  ...
  if (millis() - lastCommandTime > FAILSAFE_TIMEOUT_MS) {
    if (!isStopped || currentProbeState != PROBE_STOP) {
      Serial.println("WARNING: Communication Timeout. Engaging failsafe (All Motors Stopped).");
      stopWheels();
      currentProbeState = PROBE_STOP;
    }
  }
  ...
  if (currentProbeState == PROBE_UP) {
    probeStepper.step(1);  // Needs testing for actual physical polarity (Up vs Down)
...
// Wrapper to map relative intensities (-90 to +90) to ESC absolute PWM (0 to 180)
...
void driveWheels(int leftPWM, int rightPWM) {
  // Always limit values explicitly to prevent hardware glitches (constrain to 0-180)
